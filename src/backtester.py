...
from tools.dexscreener_api import get_crypto_prices, prices_to_df

class Backtester:
    def __init__(self, agent, token_address, chain_id, start_date, end_date, initial_capital):
        self.agent = agent
        ...
        self.portfolio = {"cash": initial_capital, "token": 0}
    ...
    def execute_trade(self, action, quantity, current_price):
        ...
        if action == "buy" and quantity > 0:
            ...
            if cost <= self.portfolio["cash"]:
                ...
                if max_quantity > 0:
                    self.portfolio["token"] += max_quantity
                    self.portfolio["cash"] -= max_quantity * current_price
                    return max_quantity
                return 0
        elif action == "sell" and quantity > 0:
            quantity = min(quantity, self.portfolio["token"])
            if quantity > 0:
    ...
    def run_backtest(self):
        ...
        print("\nStarting backtest...")
        print(f"{'Date':<12} {'Token':<6} {'Action':<6} {'Quantity':>8} {'Price':>8} {'Cash':>12} {'Token':>8} {'Total Value':>12}")
        print("-" * 100)
        ...
        for current_date in dates:
            ...
            prices = get_crypto_prices(self.token_address, self.chain_id, lookback_start, current_date_str)
            ...
            total_value = self.portfolio["cash"] + self.portfolio["token"] * current_price
            self.portfolio["portfolio_value"] = total_value

            # Log the current state with executed quantity
            print(
                f"{current_date.strftime('%Y-%m-%d'):<12} {self.token_address:<6} {action:<6} {executed_quantity:>8} {current_price:>8.2f} "
                f"{self.portfolio['cash']:>12.2f} {self.portfolio['token']:>8} {total_value:>12.2f}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--token_address', type=str, help='Token address')
